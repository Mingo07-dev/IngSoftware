...
public class PrenotazioneAutomatica {
    ...
    public static String recapitoTelefonicoFarmacia;
    public static String indirizzoFarmacia;
    public static String[] nomeFarmaciaString;

    public PrenotazioneAutomatica(){
        TimeScheduler task = new TimeScheduler(4, 2);
    }
    public static void prenotazioneAutomatica() throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        int rows = Main.dbms_Azienda.getResultSetRows(nomeFarmacia);
        nomeFarmaciaString = new String[rows];
        for(int i = 0; i < rows; i++){
            nomeFarmaciaString[i] = nomeFarmacia.getString(1);
            nomeFarmacia.next();
        }
        ...
        for(int k = 0; k < rows; k++){
            ...
            try {
                queryResult = Main.dbms_Azienda.getData("SELECT * FROM dbms_azienda.prenotazione_automatica WHERE nome_farmacia = '"+nomeFarmaciaString[k]+"';");
            } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | SQLException e) {
                e.printStackTrace();
            }
            queryResult.first();
            ...
            if(n-zeri != 0){
                ...
                try {
                    datiFarmacia = Main.dbms_Azienda.getData("SELECT * FROM dbms_azienda.farmacie WHERE nome_farmacia = '" + nomeFarmaciaString[k] + "';");
                    datiFarmacia.first();
                    recapitoTelefonicoFarmacia = datiFarmacia.getString(2);
                ...
                try {
                    Main.dbms_Azienda.setData("INSERT INTO `dbms_azienda`.`lista_ordini` (`id_ordine`, `data_consegna_ordine`, `nome_farmacia`, `stato_ordine`) VALUES ('" + id_ordine + "', '" + consegna + "', '" + nomeFarmaciaString[k] + "', '0');");
                    Main.dbms_Azienda.setData("INSERT INTO `dbms_azienda`.`elenco_consegne` (`id_ordine`, `indirizzo_postale`, `recapito_telefonico`, `data_consegna`, `nome_farmacia`, `stato_consegna`) VALUES ('" + id_ordine + "', '" + indirizzoFarmacia + "', '" + recapitoTelefonicoFarmacia + "', '" + consegna + "', '" + nomeFarmaciaString[k] + "', '0');");
                } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | SQLException ex) {
                    ex.printStackTrace();
                }
